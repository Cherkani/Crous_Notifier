...
function crousUrlAdvice(value) {
  if (!value) {
    return { tone: 'warning', text: 'Paste the search URL generated by the live Crous website after selecting a city or map zone.' }
  }
  try {
  ...
  const renderWatches = () => (
    <section className="grid two">
      <Card title="Crous search setup" icon={<Plus size={18} />}>
        <p className="section-note">Configure only the Crous targeting here. Go to the live Crous website, search by city or map zone, click “Rechercher dans la zone”, then paste the generated URL here. Receivers stay in Configuration.</p>
        <form className="form" onSubmit={submitWatch}>
          ...
          <label>Targeted Crous search URL</label>
          <input value={watchForm.url} onChange={(event) => setWatchForm({ ...watchForm, url: event.target.value })} placeholder="https://trouverunlogement.lescrous.fr/tools/.../search?bounds=..." />
          <div className={`url-advice ${watchUrlAdvice.tone}`}>
            <strong>{watchUrlAdvice.tone === 'ready' ? 'URL looks targeted' : 'URL guidance'}</strong>
            <span>{watchUrlAdvice.text}</span>
          </div>
          <div className="config-help">
            The plain URL <code>https://trouverunlogement.lescrous.fr/tools/31/search</code> is not reliable; it returned 404 during verification and does not include a target zone.
          </div>
